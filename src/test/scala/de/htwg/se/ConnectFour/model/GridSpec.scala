  ...
  "A Grid" when {
    ...
    "when not full" should {
      "have a method to replace specific cells" in {
        val replaced = grid.replaceCell(0,3, Cell(Some(impl.Piece(PlayerImpl("Your Name", 1)))))
        replaced.cell(0,3).piece should be (Some(impl.Piece(PlayerImpl("Your Name", 1))))
      }
      "have a method to put Pieces into the Grid" in {
        val dropped = grid.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
        dropped.cell(0,0).piece should be (Some(impl.Piece(PlayerImpl("Your Name", 1))))
      }
    ...
    "when full" should {
      "have no new dropped Piece" in {
        ...
        afterFull = afterFull.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
        afterFull = afterFull.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
        afterFull = afterFull.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
        afterFull = afterFull.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
        afterFull = afterFull.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
        afterFull = afterFull.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
        val newDrop = afterFull.drop(0, impl.Piece(PlayerImpl("Your Name", 1)))
    ...
    "when filled specifically" should {
      ...
      "all patterns should be checked correctly" in {
        ...
        controller.grid = controller.grid.replaceCell(0,1, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(0,2, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(0,3, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(1,0, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(2,0, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(3,0, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(1,1, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(2,2, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(3,3, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(5,6, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(4,5, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(3,4, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(2,3, Cell(Some(impl.Piece(controller.players(0)))))
        controller.currentPlayer = controller.players(0)
      ...
      "the upper left corner be checked correctly descending diagonally if a player has won the game" in {
        ...
        controller.grid = controller.grid.replaceCell(4,1, Cell(Some(impl.Piece(controller.players(1)))))
        controller.grid = controller.grid.replaceCell(3,2, Cell(Some(impl.Piece(controller.players(1)))))
        controller.grid = controller.grid.replaceCell(2,3, Cell(Some(impl.Piece(controller.players(1)))))
      ...
      "the bottom right corner be checked correctly descending diagonally if a player has won the game" in {
        ...
        controller.grid = controller.grid.replaceCell(1,5, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(2,4, Cell(Some(impl.Piece(controller.players(0)))))
        controller.grid = controller.grid.replaceCell(3,3, Cell(Some(impl.Piece(controller.players(0)))))
        controller.currentPlayer = controller.players(0)
        val descDiagonal = controller.winPatternDescendingDiagonal()
        var bool = false
